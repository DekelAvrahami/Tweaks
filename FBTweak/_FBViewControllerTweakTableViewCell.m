...
@implementation _FBViewControllerTweakTableViewCell
...
/// thread.
typedef void (^FBViewControllerTweakTableViewCellBlock)
    (_FBViewControllerTweakTableViewCell *cell, id value);

+ (NSDictionary<NSString *, FBViewControllerTweakTableViewCellBlock> *)keyPathMapping {
  return @{
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(_FBTweakTableViewCell *)cell
  ...
  FBViewControllerTweakTableViewCellBlock _Nullable block =
      [[self class] keyPathMapping][keyPath];
  if (!block) {
    return;
  }
